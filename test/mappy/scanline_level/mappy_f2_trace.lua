-- mappy_f2_trace.lua
-- Capture a detailed per-scanline / per-write PPU trace for Frame 2 of
...
--
-- Output file: C:\Work\nes1\test\mappy_out\fceux_sl_f2.txt
...
local function write_output()
    ...
    if not f then
        print("[mappy_f2_trace] ERROR: cannot open " .. OUTFILE)
        return
    end
    ...
    f:close()
    print(string.format(
        "[mappy_f2_trace] Done: %d writes logged -> %s",
        #write_log, OUTFILE))
end
