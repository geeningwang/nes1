-- mappy_f3_trace.lua
-- Capture a detailed per-scanline / per-write PPU trace for Frame 3 of
...
--
-- Output file: C:\Work\nes1\test\mappy_out\fceux_sl_f3.txt

local OUTFILE = "C:\\Work\\nes1\\test\\mappy_out\\fceux_sl_f3.txt"
local TARGET_FRAME = 3
...
local function capture_trace_callback()
    local ok, err = pcall(function()
        local d0 = ppu.getscanlinedata(0)
        local dbgf = io.open("C:/Work/nes1/test/mappy_out/lua_debug.txt", "a")
        if dbgf then
            dbgf:write(string.format("callback fired! d0=%s cycles=%s pc=%s\n",
                tostring(d0), tostring(d0 and d0.cycles), tostring(d0 and d0.pc)))
            dbgf:close()
        end
    end)
    if not ok then
        local dbgf = io.open("C:/Work/nes1/test/mappy_out/lua_debug.txt", "a")
        if dbgf then dbgf:write("ERROR: " .. tostring(err) .. "\n") dbgf:close() end
    end
    for sl = 0, 239 do
...
local function write_output()
    ...
    if not f then
        print("[mappy_f3_trace] ERROR: cannot open " .. OUTFILE)
        return
    end
    ...
    f:close()
    print(string.format(
        "[mappy_f3_trace] Done: %d writes logged -> %s",
        #write_log, OUTFILE))
end
...
recording = true
local f_pre = io.open("C:/Work/nes1/test/mappy_out/main_pre.txt", "w")
if f_pre then f_pre:write("before registerafter\n") f_pre:close() end
emu.registerafter(capture_trace_callback)
local f_pre2 = io.open("C:/Work/nes1/test/mappy_out/main_pre2.txt", "w")
if f_pre2 then f_pre2:write("before frameadvance\n") f_pre2:close() end
emu.frameadvance()
local f_post = io.open("C:/Work/nes1/test/mappy_out/main_post.txt", "w")
if f_post then f_post:write("after frameadvance\n") f_post:close() end
recording = false
